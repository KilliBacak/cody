...
loadChatHistory = data => {
	...
	for (var i = 0; i < data.length; i++){
		lastPost = data[i].log_id;
		if(!ignored(data[i].user_id) && !boomAllow(data[i].log_rank)){
			message += createChatLog(data[i]);
		}
	}
				...
				if(response.act != userAction ){
					...
					if('del' in response){
						...
						for (var i = 0; i < mainDel.length; i++){
							$("#log"+mainDel[i]).remove();
							$(".quote"+mainDel[i]).remove();
						}
					}
					if('pdel' in response){
...
logsControl = function(){
	if($('#show_chat').attr('value') == 1){
		...
		if(countDiff > 0 && countDiff % 2 === 0){
				$('#chat_logs_container').find('.chat_log:lt('+countDiff+')').remove();
				moreMain = 1;
		}
	}
}
...
postLock = function(){
	$(".post_input_container, .add_comment, .do_comment").remove();
}
doNothing = function(){
